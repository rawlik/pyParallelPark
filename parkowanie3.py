...
def circ(x, y, r, ile = 10):
    ...
    t = []
    for phi in range(0, int(2 * pow(10,p) * pi), 1):
    ...
    def move(self, t):
        ...
        if self.given_distance:
            if self.distance_left < abs(self.v * t):
                d = sign(self.v) * self.distance_left
            else:
                d = self.v * t
        else:
            d = self.v * t
        ...
        if self.given_distance:
            ...
            self.distance_left -= sign(self.v) * d
...
class AI():
    ...
    def drawSafeZone(self):
        ...
        def eval_dist(x):
            return center - x
        dist_tab = list(map(eval_dist, self.car.body))
        furthest = self.car.body[ dist_tab.index(max(dist_tab)) ]

        #trik: cmp(x,0) zwraca znak x
        t = circ(center[0], center[1], center.GetCar()[0] + self.car.body_width / 2 * sign(self.car.alpha))
        glColor3d(0,1,0)
        glBegin(GL_LINE_LOOP)
        for p in t:
            pnt = Point(self, p)
